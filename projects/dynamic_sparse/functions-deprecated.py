...
class KWinnersBatch(nn.Module):

    def __init__(self, k_perc=0.1):
        super(KWinnersBatch, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.duty_cycle is None:
            # for MLP alone
            self.duty_cycle = torch.zeros(num_units)
        ...
        with torch.set_grad_enabled(False):
            ...
            # if duty cycle, apply boosting, at training only
            threshold = self._get_threshold(tx, k)
            if self.duty_cycle is not None and self.training:
                boosting = self._calculate_boosting()
                tx *= boosting
            mask = tx > threshold
            # update duty cycle at training only
        ...
        # debug prints
        # print("mean", torch.mean(tx))
        # print("max", torch.max(tx))
        # print("threshold", threshold)
        # print("mask sum", torch.sum(mask).item()/256)
        return x * mask.float()

    def _get_threshold(self, x, k):
        """Calculate dynamic theshold""" 
        tx = x.view(-1)
        pos = int((x.shape[1] - k) * x.shape[0])
        threshold, _ = torch.kthvalue(tx, pos)
