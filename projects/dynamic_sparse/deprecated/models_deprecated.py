...
class DSNN(SparseModel):
    ...
    def _post_epoch_updates(self):
        super(DSNN, self)._post_epoch_updates()
        # run one additional testing epoch for noise
        print("In epoch updates", self.test_noise)
        if self.test_noise:
    ...
    def _run_one_pass(self, loader, train, noise=False):
        super(DSNN, self)._run_one_pass(loader, train, noise=noise)
        if train:
            # dynamically decide pruning interval
            if self.current_epoch % self.pruning_interval == 0:
                self.reinitialize_weights()

    def prune(self, weight, grads, num_params, zeta=0.35):
        """
        Calculate new weight based on SET approach weight vectorized version
        aimed at keeping the mask with the similar level of sparsity.
        ...
        with torch.no_grad():
            ...
            weight_prune_mask = ((weight >= pos_threshold) | (weight <= neg_threshold))
            weight_prune_mask.to(self.device)

            # calculate gradient mask
            deltas = (grads * torch.sign(weight)).view(-1)
            grads_treshold, _ = torch.kthvalue(deltas, int(zeta * len(deltas)))
            grads_prune_mask = (grads >= grads_treshold).to(self.device)

            # combine both masks
            prune_mask = weight_prune_mask & grads_prune_mask
    ...
    def reinitialize_weights(self):
        ...
        for idx, m in enumerate(self.sparse_modules):
            new_mask, prune_mask, new_synapses = self.prune(
                m.weight.clone().detach(), m.weight.grad, self.num_params[idx]
            )
            with torch.no_grad():
                self.masks[idx] = new_mask.float()
                m.weight.data *= prune_mask.float()
