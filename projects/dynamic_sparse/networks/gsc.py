...
def make_dscnn(net, config={}):
    ...
    def get_conv_type(prune_method):
        # COMMENT: avoid using \ in Python, last case scenario
        # pythonic way: regular if else, easier to read
    ...
    # COMMENT:
    # unnecessarily complex operation, super hard to read!
    # a lot can be done to simplify - make it simpler - not more complex
    # also has to fit in the style of the repo you are pushing to

    possible_args = {
        "dynamic": ["hebbian_prune_frac",
                    "weight_prune_frac",
                    "sparsity",
                    "prune_dims",
                    "update_nsteps"],
        "random": ["hebbian_prune_frac",
                    "weight_prune_frac",
                    "sparsity",
                    "prune_dims",
                    "update_nsteps"],
    ...
    assert len((kwargs_s)) == len(named_convs) == len(prune_methods), "Sizes do not match"
